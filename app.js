...
function render() {
  ...
  // the preview is what the carousel looks like: nothing on top of the photos, no invite, no "+ slide"
  $('#add-slide-here').classList.toggle('hidden', mode !== 'edit');
  if (mode === 'edit') renderStrip(); else { $('.strip-empty')?.remove(); renderPreviewTrack(); }
  renderRuler();
  renderLayers();
  renderInspector();
  scheduleAutosave();
