...
function styleItem(node, item, width) {
  ...
  box.style.borderRadius = `${framePx(item.radius * 3, width)}px`;
...
async function autoLayout(files) {
  ...
  // 1. a template's empty frames come first: that is what they are there for
  ...
  // 2. the rest is arranged over the slides that already exist, never adding any
  const left = sources.slice(used);
  if (left.length) {
    const freeSlides = Array.from({ length: project.slideCount }, (_, i) => i)
      .filter(i => !project.items.some(item => insideSlide(item, i) || (item.x < i + 1 && item.x + item.w > i)));
    const targets = freeSlides.length ? freeSlides : Array.from({ length: project.slideCount }, (_, i) => i);
    const perSlide = spread(Math.min(left.length, targets.length * MAX_PER_SLIDE), targets.length);
    const style = pick(STYLES);
    let taken = 0, previous = null;
    targets.forEach((slide, position) => {
      const count = perSlide[position];
      if (!count) return;
  ...
  toast(leftOver
    ? `${used} foto sistemate su ${project.slideCount} slide — ${leftOver} non entrano: aggiungi slide e ripeti`
    : frames.length && used === frames.length
      ? `${used} foto inserite nei riquadri del template`
      : `${used} foto disposte su ${project.slideCount} slide, stile ${lastComposition || 'pulito'}`);
}
...
function stripHTML(items, count, offset) {
  const shots = items.map(i => {
    ...
    return `<div style="position:absolute;left:${i.x / count * 100}%;top:${i.y * 100}%;width:${i.w / count * 100}%;height:${i.h * 100}%;
      ...
      border-radius:${i.radius / 5}px;box-shadow:${i.frame === 'none' ? 'none' : '0 3px 8px #0007'};transform:rotate(${i.rotation}deg)"></div>`;
  }).join('');
...
function drawItem(ctx, item, image, index) {
  ...
  if (spec.border) {
    ...
    roundedRect(ctx, -bw / 2, -bh / 2, bw, bh, radius);
    ctx.fill();
  ...
  roundedRect(ctx, ix, iy, iw, ih, Math.max(0, radius - spec.border));
  ctx.clip();
